...
typedef struct fd_info_s {
        struct fd_info_s *next;
        int fd;
        char type[16]; /* "socket" or "file" */
        char *filename;
        int inode;
} fd_info_t;
...
typedef struct process_s {
        ...
        unsigned long long u_time;  /* running time in user mode in seconds */
        unsigned long long s_time;  /* running time in system mode in seconds */
        list_head_t fdlist; /* get from /proc/{process_id}/fd */
        list_head_t tcp_socklist; /* get from /proc/net/tcp */
        list_head_t udp_socklist; /* get from /proc/net/tcp */
        char *cmdline; /* get from /proc/{process_id}/cmdline */
} process_t;


int get_process_list(char *process_name, list_head_t *proclist, int detail);
int free_process_list(list_head_t *list);
