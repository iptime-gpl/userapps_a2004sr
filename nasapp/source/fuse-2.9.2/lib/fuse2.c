...
int syslog2( char *fmt, ...)
{
        FILE *fp;
        va_list args;
        char syslog_file[128];

        strcpy(syslog_file,"/tmp/msg");
        ...
        /* appened temp file */
        append_file_line(fp, syslog_file, 100);
        fclose(fp);
...
int fuse_fs_mknod(struct fuse_fs *fs, const char *path, mode_t mode,
		  dev_t rdev)
{
syslog2("fuse_fs_mknod 1");
	fuse_get_context()->private_data = fs->user_data;
syslog2("fuse_fs_mknod 2");
	if (fs->op.mknod) {
	int ret;
		if (fs->debug)
...
syslog2("fuse_fs_mknod 3");
#if	0
		return fs->op.mknod(path, mode, rdev);
#else
		ret= fs->op.mknod(path, mode, rdev);
		syslog2("fuse_fs_mknod 4");
		return ret;
#endif
	} else {
		syslog2("fuse_fs_mknod 5");
