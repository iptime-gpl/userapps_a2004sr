...
sigint_handler(int sig)
{
     ...
     fp = fopen("/var/run/Status/Plugin.set", "w+");
     if (fp)
     {
	     fprintf(fp, "package=%s\n", EFM_package);
	     fprintf(fp, "status=cancel\n");
	     fclose(fp);
     }

     // remove tmp_dir
     /*
     printf("remove tmp_dir[%s]\n", conf->tmp_dir);
     system2("/bin/rm -rf \"%s/opkg-*\"", conf->tmp_dir);
     */
#endif // EFM_PATCH
     exit(128 + sig);
}
...
opkg_install_cmd(int argc, char **argv)
{
     ...
     for (i=0; i < argc; i++) {
	  arg = argv[i];
#ifdef EFM_PATCH // set pakcage name - arg
	  strcpy(EFM_package, arg);
#endif // EFM_PATCH
...
#ifdef EFM_PATCH
     //update_opkg_status(EFM_OPKG_STATUS_DONE);
     if (!EFM_sigint)
     {
	     iconfig_set_value("Status:Plugin", "package", EFM_package);
	     iconfig_set_value("Status:Plugin", "status", "done");
...
opkg_upgrade_cmd(int argc, char **argv)
{
     ...
     if (argc) {
	  ...
	  for (i=0; i < argc; i++) {
	       ...
	       if (pkg) {
#ifdef EFM_PATCH // set pakcage name - pkg->name
		       strcpy(EFM_package, pkg->name);
#endif // EFM_PATCH
...
#ifdef EFM_PATCH
     iconfig_set_value("Status:Plugin", "package", pkg_to_remove->name);
     iconfig_set_value("Status:Plugin", "status", "removed");
     iconfig_set_value("Status:Plugin", "progress", "");
#endif // EFM_PATCH
