...
opkg_install_by_name(const char *pkg_name)
{
	...
	return -1;
     ...
     opkg_msg(DEBUG2,"Calling opkg_install_pkg.\n");
#ifdef EFM_PATCH
     return opkg_install_pkg_wrap(new, 0);
#else
     return opkg_install_pkg(new, 0);
#endif // EFM_PATCH
}

#ifdef EFM_PATCH // add functions to calculate total package size.
double EFM_total_size;
double EFM_current_size;
char EFM_package[1024];
int EFM_sigint;

static int EFM_satisfy_dependencies_for(pkg_t *pkg);

static int
EFM_opkg_install_pkg(pkg_t *pkg, int from_upgrade)
{
     int err = 0;
     int message = 0;
     pkg_t *old_pkg = NULL;
     ...
     // if pkg is already installed, so check whether dependcies are installed
     if (pkg->state_status == SS_INSTALLED && conf->nodeps == 0) {
	  err = EFM_satisfy_dependencies_for(pkg);
	  if (err)
		  return -1;
     ...
     // check whether pkg is installed and version.
	...
	uncecessary upgrade */
     ...
     if (pkg->local_filename == NULL) {
	     ...
	     {
		// update EFM_total_size with pkg.
                EFM_total_size += EFM_opkg_download_pkg(pkg);
	     }
             else
                return -1;
         } else {
...
static int
EFM_satisfy_dependencies_for(pkg_t *pkg)
{
     int i;
     pkg_vec_t *depends = pkg_vec_alloc();
     pkg_t *dep;
...
int
opkg_install_pkg_wrap(pkg_t *pkg, int from_upgrade)
{
	int ret;

	// initialize EFM_total_size.
	EFM_total_size = 0;
	EFM_current_size = 0;
	
	// update total size.
	EFM_opkg_install_pkg(pkg, from_upgrade);

	ret = opkg_install_pkg(pkg, from_upgrade);
...
opkg_install_pkg(pkg_t *pkg, int from_upgrade)
{
	...
	uncecessary upgrade */
     ...
     iconfig_set_value("Status:Plugin", "status", "install");
     iconfig_set_value("Status:Plugin", "progress", ""); 
#endif // EFM_PATCH
