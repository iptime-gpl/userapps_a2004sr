...
int iconfig_set_value(char *property,char *tag, char *value)
{
        char real_tag[128];

        sprintf(real_tag,"opkg_%s",tag);
        return istatus_set_value_direct(tag,value);
}

int iconfig_set_intvalue(char *property,char *tag, int value)
{
        char intstr[64];

        sprintf(intstr,"%d",value);
        return iconfig_set_value(property,tag,intstr);
}
#endif
...
extern double EFM_current_size;
extern double EFM_total_size;
extern char EFM_package[1024];

int
update_opkg_status(char *msg) 
{
	//printf("%s\n", msg);

	FILE *fp;
	fp = fopen(EFM_OPKG_STATUS, "w");
	if (fp)
	{
	    fprintf(fp, "%s", msg);
	    fclose(fp);
...
// download progress.
int
EFM_progress_cb(struct _curl_cb_data *cb_data, double t,	/* dltotal */
		double d,	/* dlnow */
		double ultotal, double ulnow)
{
	...
	if (d == 0.0)
	    ...
	    EFM_current_size += (d - d1);

	progress = (int)((EFM_current_size / EFM_total_size) * 100);
	//progress = (int)((d / t) * 100);
	/*
	sprintf(msg, "package=%s\nstatus=download\nprogress=%d\n", EFM_package, progress);
	update_opkg_status(msg);
	*/

	iconfig_set_value("Status:Plugin", "package", EFM_package);
	iconfig_set_value("Status:Plugin", "status", "download");
	iconfig_set_intvalue("Status:Plugin", "progress", progress);
...
double
EFM_opkg_download_pkg(pkg_t *pkg) // get pkg size.
{
    char *url;
...
#ifdef EFM_PATCH
    //err = opkg_download_cache(url, pkg->local_filename, (curl_progress_func)EFM_progress_cb, (void *)&EFM_current_size);
    err = opkg_download_cache(url, pkg->local_filename, (curl_progress_func)EFM_progress_cb, NULL);
#else
